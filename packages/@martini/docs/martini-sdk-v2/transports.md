...
`@martini/core` doesn’t care how messages travel. Any module that implements the transport interface can plug into the runtime. This guide explains the built-in adapters and how to pick the right one.
...
```
Package: @martini/transport-ws (coming soon)
...
```
Package: @martini/transport-udp (planned)
...
## Using Martini with Existing Platforms

You can plug Martini into other ecosystems by building thin transport adapters:

- **Colyseus** – Wrap their WebSocket/Room API and let Martini handle logic. Server becomes a “dumb relay.”
- **Nakama** – Use Nakama’s realtime messaging to shuttle Martini wire messages.
...
- Need more context on where Martini fits? Read [platform-comparison.md](./platform-comparison.md).
