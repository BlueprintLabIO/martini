/**
 * Example WebSocket Server for Martini
 *
...
class MartiniServer {
  ...
  constructor(port: number) {
    ...
    this.wss.on('connection', (ws) => {
      console.log('[MartiniServer] New connection');
      ...
      ws.on('message', (data) => {
        try {
          ...
          console.error('[MartiniServer] Failed to parse message:', error);
        }
      });
      ...
      ws.on('close', () => {
        ...
        if (player) {
          console.log(`[MartiniServer] Player ${player.id} disconnected`);
          this.handlePlayerDisconnect(player);
        }
      });

      ws.on('error', (error) => {
        console.error('[MartiniServer] WebSocket error:', error);
      });
    });
  }
  ...
  private handleHandshake(ws: WebSocket, message: any): void {
    ...
    console.log(`[MartiniServer] Player ${playerId} connected`);
  ...
  private handleJoinRoom(ws: WebSocket, message: any): void {
    ...
    console.log(`[MartiniServer] Player ${player.id} joined room ${roomId}`);

    // Elect host if this is the first player
    if (!room.hostId) {
      room.hostId = player.id;
      console.log(`[MartiniServer] Player ${player.id} is now host of room ${roomId}`);
    }
  ...
  private removePlayerFromRoom(player: Player, room: Room): void {
    ...
    console.log(`[MartiniServer] Player ${player.id} left room ${room.id}`);
    ...
    if (room.players.size === 0) {
      ...
      console.log(`[MartiniServer] Room ${room.id} deleted (empty)`);
      return;
    }
    ...
    if (wasHost) {
      ...
      console.log(`[MartiniServer] Player ${newHostId} is now host of room ${room.id}`);
