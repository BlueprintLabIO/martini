 ...
 *
 * This test starts the actual MartiniServer and creates multiple clients
...
describe('WebSocketTransport Integration', () => {
  let server: MartiniTestServer;
  const PORT = 9876;
  const SERVER_URL = `ws://localhost:${PORT}`;

  beforeAll(() => {
    server = new MartiniTestServer(PORT);
  });
