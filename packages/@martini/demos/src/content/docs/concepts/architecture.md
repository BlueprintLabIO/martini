---
title: Architecture
description: Understanding Martini's host-authoritative multiplayer architecture
section: concepts
order: 1
---
...
Martini uses a **host-authoritative** architecture where one player (the host) runs the authoritative game logic and all other players (clients) mirror that state. This is simpler than deterministic lockstep and works seamlessly with existing physics engines.

## Core Philosophy

### Host-Authoritative Pattern

In Martini, the **host is the source of truth**:
...
|--------------|--------------|------|------|
| **Host-Authoritative (Martini)** | Host runs game, clients mirror | Simple, works with any engine, no sync bugs | Slight input latency for clients |
...
Martini is composed of several packages, each with a specific role:
...
**Key Types**:
```typescript
PhaserAdapter       // Bridge between Martini and Phaser
...
**Packages**:
- `@martini/transport-local` - In-memory (for demos/testing, 0ms latency)
- `@martini/transport-iframe-bridge` - Iframe-based (for IDE, ~1ms latency)
- `@martini/transport-trystero` - P2P WebRTC (for production, 20-100ms latency)
...
└──────────────────────────┼──────────────────────────────────┘
          ...
          ▼                ▼                ▼
┌─────────────────┐  ┌──────────────┐  ┌─────────────┐
│ @martini/phaser │  │  Transport   │  │  DevTools   │
│                 │  │  Interface   │  │             │
...
- Martini: Slight latency, but simple
...
**When to use which:**
- Small co-op games: Martini (P2P)
- Competitive games with many players: Server-authoritative

<Callout type="tip" title="Martini Can Be Server-Authoritative!">

Martini supports server-authoritative mode by making the "host" a dedicated Node.js server instead of a player's client. The architecture is identical.
...
Martini doesn't care how messages are sent:
...
`@martini/core` has **zero dependencies on game engines**. It works with:

- Phaser (via `@martini/phaser`)
