---
title: Actions
description: Understanding Martini's action system and the critical playerId vs targetId distinction
section: concepts
order: 3
---
...
Actions are **how you change game state** in Martini. They are the bridge between player input and state mutations. Understanding actions—especially the difference between `playerId` and `targetId`—is critical.
...
Martini provides helpers to reduce boilerplate:
...
```typescript
import { createTickAction } from '@martini/core';
