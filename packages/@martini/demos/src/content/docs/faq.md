...
description: Common questions about Martini SDK
---
...
Martini SDK is an engine-agnostic, host-authoritative multiplayer framework for building real-time multiplayer games. It provides a declarative API where you define your game's state and actions, and Martini handles all the networking complexity.

### Is Martini SDK production-ready?

Yes! Martini SDK is currently in v0.1 and suitable for production use. Choose an appropriate transport for your use case:
- **P2P games**: Use `@martini/transport-trystero`
- **Server-based games**: Use `@martini/transport-ws` or implement a custom transport
- **Testing**: Use `@martini/transport-local`

### What license is Martini SDK under?

Martini SDK is open source under the MIT license.

### Who maintains Martini SDK?

Martini SDK is maintained by the core team and community contributors. See [CONTRIBUTORS.md](https://github.com/yourusername/martini/blob/main/CONTRIBUTORS.md) for the full list.
...
### What game engines does Martini support?

**Official adapters:**
- **Phaser 3** - Full integration with `@martini/phaser`
- **Vanilla JavaScript** - Use `@martini/core` directly
...
### Does Martini work with React/Vue/Svelte?

Yes! You can embed Martini games in any framework:
...
Martini supports all modern browsers:
...
Martini is written in TypeScript and designed for TypeScript-first development.

## Development Questions

### How do I test my multiplayer game locally?

Use `@martini/transport-local` for instant testing:
...
### Can I use Martini with Vite/Webpack/Parcel?

Yes! Martini works with any bundler that supports ES modules:
- **Vite** (recommended)
- Webpack 5+
- Parcel 2+
- Rollup
...
Use the `StateInspector` from `@martini/devtools`:
...
**Official transports:**
- `@martini/transport-local` - In-memory (testing)
- `@martini/transport-iframe-bridge` - Iframe communication (IDE)
- `@martini/transport-trystero` - P2P WebRTC
- `@martini/transport-ws` - WebSocket (server-based)
- `@martini/transport-colyseus` - Colyseus integration

See [Transport Overview](/docs/latest/api/transports/overview) for comparison.

### Can I use Martini without a server?

Yes! Use `@martini/transport-trystero` for P2P games:
- No server costs
- WebRTC peer-to-peer
...
Implement the `Transport` interface from `@martini/core`:
...
### Does Martini support dedicated servers?
...
### Does Martini support voice chat?

Not built-in, but you can integrate:
- Use WebRTC data channels alongside Martini
...
### How does Martini compare to Colyseus?

| Feature | Martini | Colyseus |
|---------|---------|----------|
...
**Use Martini if:** You want P2P support, Phaser integration, or transport flexibility

**Use Colyseus if:** You need battle-tested server infrastructure

### How does Martini compare to Photon?

| Feature | Martini | Photon |
|---------|---------|--------|
...
**Use Martini if:** You want open source, no vendor lock-in, free hosting

**Use Photon if:** You need cross-platform support with hosted infrastructure

### How does Martini compare to Mirror (Unity)?

Mirror is Unity-specific. Martini is engine-agnostic (currently Phaser, Unity planned).

**Use Martini if:** Building web games
...
[Open an issue on GitHub](https://github.com/yourusername/martini/issues) with:
- Steps to reproduce
...
Post in [GitHub Discussions](https://github.com/yourusername/martini/discussions/categories/feature-requests):
...
- **Community**: Ask in [GitHub Discussions](https://github.com/yourusername/martini/discussions)
- **Issues**: Report bugs on [GitHub Issues](https://github.com/yourusername/martini/issues)
