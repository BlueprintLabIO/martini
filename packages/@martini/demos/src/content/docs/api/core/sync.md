...
Martini uses a **diff/patch algorithm** to efficiently synchronize game state between the host and clients. Instead of sending the full state every frame, only the **changes** (diffs) are transmitted, drastically reducing bandwidth.
...
```typescript
import { generateDiff, applyPatch, deepClone } from '@martini/core';
...
```typescript
import { deepClone, applyPatch, generateDiff } from '@martini/core';
