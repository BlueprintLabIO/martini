...
The `StateInspector` is a powerful development tool for debugging Martini multiplayer games. It provides real-time monitoring of game state changes, action history tracking, and performance statistics.
...
```bash
pnpm add -D @martini/devtools
# or
npm install --save-dev @martini/devtools
```
...
- Diffs are computed using Martini's built-in `generateDiff()` algorithm
