...
class MockGameRuntime {
    ...
    simulateEvent(eventName, senderId, payload) {
        const callbacks = this.eventCallbacks.get(eventName) || [];
...
describe('PhaserAdapter', () => {
    describe('Initialization', () => {
        it('initializes with runtime and scene', () => {
            const runtime = new MockGameRuntime(true);
        ...
        it('registers onChange listener', () => {
            ...
            runtime.simulateStateChange({ _sprites: { 'test': { x: 100 } } });
            // Should not throw
        });
    });
    describe('myId', () => {
    ...
    describe('trackSprite (Host)', () => {
        it('tracks sprite position', () => {
            ...
            const scene = new MockScene();
            const adapter = new PhaserAdapter(runtime, scene);
            const sprite = { x: 100, y: 200 };
            adapter.trackSprite(sprite, 'player-1');
        ...
        it('starts sync loop on first tracked sprite', () => {
            ...
            const adapter = new PhaserAdapter(runtime, scene);
            const sprite = { x: 100, y: 200 };
            adapter.trackSprite(sprite, 'player-1', { syncInterval: 100 });
            // Move sprite
            sprite.x = 150;
            sprite.y = 250;
            // Clear initial state
            runtime.state._sprites = {};
            // Advance time
    ...
    describe('Remote Sprites (Client)', () => {
        it('registers remote sprite for interpolation', () => {
            ...
            const adapter = new PhaserAdapter(runtime, scene);
            const sprite = { x: 100, y: 200 };
            adapter.registerRemoteSprite('player-2', sprite);
            // Should not throw
        ...
        it('snaps to position on first update', () => {
            ...
            const scene = new MockScene();
            const adapter = new PhaserAdapter(runtime, scene);
            const sprite = { x: undefined, y: undefined };
            adapter.registerRemoteSprite('player-2', sprite);
            // First update
            runtime.simulateStateChange({
                _sprites: {
    ...
    describe('updateInterpolation', () => {
        ...
        it('interpolates all remote sprites on client', () => {
            ...
            const sprite2 = { x: 0, y: 0 };
            adapter.registerRemoteSprite('player-2', sprite1);
            adapter.registerRemoteSprite('player-3', sprite2);
    ...
    describe('Custom Events', () => {
        ...
        it('listens for events', () => {
            ...
            // Simulate event
            runtime.simulateEvent('explosion', 'p2', { x: 100, y: 200 });
    ...
    describe('onChange', () => {
        it('wraps runtime onChange', () => {
            ...
            runtime.simulateStateChange({ test: 'value' });
    ...
    describe('Edge Cases', () => {
        it('handles tracking multiple sprites', () => {
            ...
            const scene = new MockScene();
            const adapter = new PhaserAdapter(runtime, scene);
            const sprite1 = { x: 100, y: 200 };
            const sprite2 = { x: 300, y: 400 };
            const sprite3 = { x: 500, y: 600 };
            adapter.trackSprite(sprite1, 'player-1');
            adapter.trackSprite(sprite2, 'player-2');
            adapter.trackSprite(sprite3, 'bullet-1');
            const state = runtime.getState();
