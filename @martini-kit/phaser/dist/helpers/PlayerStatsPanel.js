 ...
 */
export function createPlayerStatsPanel(adapter, scene, config) {
    ...
    const update = () => {
        ...
        for (const [statName, statConfig] of visibleStats) {
            ...
            if (!element) {
                ...
                const valueText = scene.add.text(style.padding + style.iconSize + 4, currentY, String(statConfig.getValue(myPlayer)), {
                    fontSize: style.fontSize,
                    color: '#ffffff'
                });
                // Highlight rectangle (behind text, shown conditionally)
                const highlight = scene.add.rectangle(0, currentY + style.iconSize / 2, 0, style.iconSize + 4, parseInt(style.highlightColor.replace('#', '0x'), 16), 0.3);
...
//# sourceMappingURL=PlayerStatsPanel.js.map