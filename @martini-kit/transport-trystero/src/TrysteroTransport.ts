...
import { joinRoom, type Room, selfId } from 'trystero/mqtt';
...
export class TrysteroTransport implements Transport {
  ...
  private sendWire: (msg: WireMessage, peerId?: string) => void;
  private receiveWireCleanup: () => void;

  private permanentHost: string | null = null; // Set once, never changes
  ...
  constructor(options: TrysteroTransportOptions) {
    ...
    const cleanup = receive((message, peerId) => {
      ...
      if ('type' in message && message.type === 'host_announce') {
        ...
        const announcedHost = (message as any).hostId;
