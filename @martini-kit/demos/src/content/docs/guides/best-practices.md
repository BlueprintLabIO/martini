...
description: Performance optimization, debugging techniques, and architectural patterns for martini-kit multiplayer games
section: guides
order: 2
---
...
This guide covers performance optimization, debugging techniques, state design patterns, and production deployment best practices for martini-kit games.
...
**Why?** martini-kit's diff algorithm compares previous state to current state. Fewer properties = faster diffs = better performance.
...
**Why?** When an array changes, martini-kit has to diff the entire array. With records, only changed entities are diffed.
...
```typescript
import { PhaserAdapter, enableDebugLogging } from '@martini-kit/phaser';
...
- **[@martini-kit/core API](/docs/api/core)** - Core API reference
- **[@martini-kit/phaser API](/docs/api/phaser)** - Phaser API reference
