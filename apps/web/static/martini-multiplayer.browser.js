...
var MartiniMultiplayer = (() => {
  ...
  function defineGame(definition) {
    ...
    for (const [name2, action] of Object.entries(definition.actions)) {
      if (typeof action.apply !== "function") {
        throw new Error(`Action "${name2}" must have an apply function`);
      }
    }
    return definition;
  }
  ...
  var PhaserAdapter = class {
    ...
    trackedSprites = /* @__PURE__ */ new Map();
    remoteSprites = /* @__PURE__ */ new Map();
    ...
    updateSpritesFromState(state) {
      if (this.isHost() || !state._sprites) return;
