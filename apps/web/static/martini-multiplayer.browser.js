...
var MartiniMultiplayer = (() => {
  ...
  __export(browser_exports, {
    PhaserMultiplayerRuntime: () => PhaserMultiplayerRuntime,
    TrysteroTransport: () => TrysteroTransport
  });

  // ../multiplayer/dist/determinism/seeded-random.js
  var SeededRandom = class _SeededRandom {
     ...
     *
     * @example
     * rng.range(1, 7); // 1-6 (dice roll)
     * rng.range(0, 100); // 0-99
     */
    range(min, max) {
      if (min >= max) {
     ...
     *
     * @example
     * rng.choice(['rock', 'paper', 'scissors']); // 'paper'
     */
    choice(arr) {
      if (arr.length === 0) {
     ...
     * rng.shuffle(deck); // [3, 1, 5, 2, 4] (deterministic)
     */
    shuffle(arr) {
     ...
     * Clone the RNG state
     *
     * Useful for saving/restoring random state in snapshots.
     *
     * @example
     ...
     * rng.next(); // 0.8765...
     * clone.next(); // 0.8765... (same as original)
     */
    clone() {
      const clone = new _SeededRandom(0);
      clone.state = this.state;
      return clone;
    }
  };
  function createActionRandom(tick, actionIndex) {
    const seed = tick * 999983 + actionIndex;
    return new SeededRandom(seed);
  }
  function createSystemRandom(tick, systemName) {
    let hash = 0;
    for (let i = 0; i < systemName.length; i++) {
      hash = (hash << 5) - hash + systemName.charCodeAt(i);
      hash = hash & hash;
    }
    const seed = tick * 999983 + hash;
    return new SeededRandom(seed);
  }
  function createPlayerRandom(tick, playerId) {
    let hash = 0;
  ...
  // ../multiplayer/dist/utils/deep.js
  function deepClone(obj, seen = /* @__PURE__ */ new WeakMap()) {
    ...
    if (Object.getPrototypeOf(obj) !== Object.prototype) {
      console.warn("deepClone: Non-plain object detected, returning reference:", obj);
      return obj;
    }
    const clone = {};
    seen.set(obj, clone);
  ...
  // ../multiplayer/dist/schema/schema-proxy.js
  var proxyCache = /* @__PURE__ */ new WeakMap();
  var pathMap = /* @__PURE__ */ new WeakMap();
  function createSchemaProxy(state, schema, parentPath = []) {
      ...
      set(target, prop, value) {
        ...
        if (schemaRule) {
          ...
          if (actualType !== schemaRule.type) {
            throw new Error(`Schema violation: ${pathString} expects ${schemaRule.type}, got ${actualType}`);
          }
          if (schemaRule.enum !== void 0) {
            if (!schemaRule.enum.includes(value)) {
              throw new Error(`Schema violation: ${pathString} must be one of [${schemaRule.enum.join(", ")}], got ${value}`);
            }
          }
          if (schemaRule.type === "number") {
            if (schemaRule.min !== void 0 && value < schemaRule.min) {
              if (schemaRule.strict) {
                throw new Error(`Schema violation: ${pathString} must be >= ${schemaRule.min}, got ${value}`);
              }
              value = schemaRule.min;
            }
            if (schemaRule.max !== void 0 && value > schemaRule.max) {
              if (schemaRule.strict) {
                throw new Error(`Schema violation: ${pathString} must be <= ${schemaRule.max}, got ${value}`);
              }
              value = schemaRule.max;
            }
          }
        }
  ...
  function matchSchemaPath(schema, pathString) {
    const parts = pathString.split(".");
    if (schema[pathString]) {
      return schema[pathString];
    }
    for (const pattern in schema) {
      if (pattern === pathString)
        continue;
  ...
  function partsMatch(parts, pattern) {
  ...
  // ../multiplayer/dist/action/action-executor.js
  var ActionExecutor = class {
    tickDuration;
    cooldowns = /* @__PURE__ */ new Map();
    // playerId -> actionName -> lastTick
    rateLimitTracker = /* @__PURE__ */ new Map();
    // playerId -> actionName -> ticks[]
    currentTick = 0;
    actionIndex = 0;
    ...
    /**
     * Set current tick - resets actionIndex only when tick changes
     */
    setTick(tick) {
    ...
    /**
     * Get next actionIndex for manual tracking
     */
    nextActionIndex() {
      return this.actionIndex++;
    }
    /**
     * Execute an action with full validation pipeline
     *
     * @param actionName - Action identifier
     * @param payload - Action input (will be validated)
     * @param playerId - Player executing the action
     * @param state - Mutable game state
     * @param config - Action configuration
     * @param actionIndexOverride - Use specific index (for replays)
     * @returns Result with success/rejection status
     */
    executeAction(actionName, payload, playerId, state, config, actionIndexOverride) {
      if (config.input) {
      ...
      if (config.requires) {
        const meetsRequirements = this.checkRequirements(config.requires, playerId, state, payload, actionName);
    ...
    /**
     * Validate input against schema (with clamping)
     */
    validateInput(payload, inputSchema) {
      const validated = {};
      for (const key in inputSchema) {
        const rule = inputSchema[key];
        const value = payload[key];
        const schemaRule = typeof rule === "string" ? { type: rule } : rule;
        const actualType = Array.isArray(value) ? "array" : typeof value;
        if (actualType !== schemaRule.type) {
          return {
            valid: false,
            error: `Invalid input: ${key} expects ${schemaRule.type}, got ${actualType}`
          };
        }
        ...
        if (schemaRule.type === "number") {
          if (schemaRule.min !== void 0 && value < schemaRule.min) {
            if (schemaRule.strict) {
              return {
                valid: false,
                error: `Invalid input: ${key} must be >= ${schemaRule.min}, got ${value}`
              };
            }
            finalValue = schemaRule.min;
          }
          if (schemaRule.max !== void 0 && value > schemaRule.max) {
            if (schemaRule.strict) {
              return {
                valid: false,
                error: `Invalid input: ${key} must be <= ${schemaRule.max}, got ${value}`
              };
            }
            finalValue = schemaRule.max;
          }
        }
        if (schemaRule.enum !== void 0) {
          if (!schemaRule.enum.includes(value)) {
            return {
              valid: false,
              error: `Invalid input: ${key} must be one of [${schemaRule.enum.join(", ")}], got ${value}`
            };
          }
        }
    ...
    /**
     * Check action requirements (cooldown, rate limit, proximity, custom)
     */
    checkRequirements(requires, playerId, state, payload, actionName) {
      if (requires.cooldown) {
        const cooldownTicks = Math.ceil(requires.cooldown / this.tickDuration);
        if (!this.cooldowns.has(playerId)) {
          this.cooldowns.set(playerId, /* @__PURE__ */ new Map());
        }
        const lastTick = this.cooldowns.get(playerId).get(actionName);
        if (lastTick !== void 0 && this.currentTick - lastTick < cooldownTicks) {
          const remainingTicks = cooldownTicks - (this.currentTick - lastTick);
          return {
            valid: false,
            error: `Cooldown: ${remainingTicks} ticks (~${Math.ceil(remainingTicks * this.tickDuration)}ms) remaining`
          };
        }
        this.cooldowns.get(playerId).set(actionName, this.currentTick);
      }
      if (requires.rateLimit) {
        const { max, window: window2 } = requires.rateLimit;
        const windowTicks = Math.ceil(window2 / this.tickDuration);
        if (!this.rateLimitTracker.has(playerId)) {
          this.rateLimitTracker.set(playerId, /* @__PURE__ */ new Map());
        }
        ...
        if (recentCalls.length >= max) {
          return {
            valid: false,
            error: `Rate limit: max ${max} calls per ${window2}ms`
          };
        }
      ...
      if (requires.custom) {
        const result = requires.custom({ game: state, input: payload, playerId });
        if (result !== true) {
          return {
            valid: false,
  ...
  // ../multiplayer/dist/action/system-executor.js
  var SystemExecutor = class {
    tickRate;
    tickDuration;
    currentTick = 0;
    constructor(tickRate, tickDuration) {
    ...
    /**
     * Execute all systems for the current tick
     *
     * @param state - Mutable game state
     * @param systems - System configurations
     */
    executeSystems(state, systems) {
      for (const systemName in systems) {
        const config = systems[systemName];
        if (!config.rate || config.rate <= 0) {
          continue;
        }
        const executionsPerTick = config.rate / this.tickRate;
        if (executionsPerTick >= 1) {
          const count = Math.floor(executionsPerTick);
          for (let i = 0; i < count; i++) {
            this.executeSystem(systemName, config, state);
          }
        } else {
          const tickInterval = Math.floor(this.tickRate / config.rate);
          const shouldSkipTickZero = tickInterval >= this.tickRate;
          const isMultiple = this.currentTick % tickInterval === 0;
          if (isMultiple && (!shouldSkipTickZero || this.currentTick > 0)) {
            this.executeSystem(systemName, config, state);
          }
        }
      }
    ...
    /**
     * Execute a single system once
     */
    executeSystem(systemName, config, state) {
      const context = {
        game: state,
        dt: 1 / config.rate,
        // Delta time based on system rate
        random: createSystemRandom(this.currentTick, systemName),
        time: this.currentTick * this.tickDuration
      };
      try {
        config.tick(context);
      } catch (error) {
        console.error(`System "${systemName}" error:`, error);
      }
    }
  };

  // ../multiplayer/dist/runtime/multiplayer-runtime.js
  var MultiplayerRuntime = class {
    transport;
    // Executors
    actionExecutor;
    systemExecutor;
    ...
    constructor(gameLogic, transport, config) {
      this.transport = transport;
      this.gameLogic = gameLogic;
      this.config = config;
      this.tickRate = config.tickRate || 30;
      this.tickDuration = 1e3 / this.tickRate;
      this.isHost = !!config.isHost;
      const providedPlayerIds = config.playerIds;
      const initialPlayerIds = providedPlayerIds ? [...providedPlayerIds] : [];
      this.playerIds = new Set(initialPlayerIds);
      this.actionExecutor = new ActionExecutor(this.tickDuration);
      this.systemExecutor = new SystemExecutor(this.tickRate, this.tickDuration);
      const shouldDeferSetup = this.isHost && providedPlayerIds === void 0;
      if (!shouldDeferSetup) {
        this.initializeState(initialPlayerIds);
        this.invokeInitialPlayerHooks(initialPlayerIds);
      }
      this.registerTransportListeners();
      this.actionExecutor.setTick(0);
      this.systemExecutor.setTick(0);
    }
    initializeState(playerIds) {
      const initialState = this.gameLogic.setup({
        playerIds,
        time: this.tick * this.tickDuration
      });
      this.state = this.gameLogic.schema ? createSchemaProxy(initialState, this.gameLogic.schema) : initialState;
    }
    invokeInitialPlayerHooks(playerIds) {
      if (!this.gameLogic.onPlayerJoin || !this.state) {
        return;
      }
      for (const playerId of playerIds) {
        this.invokePlayerJoinHook(playerId);
      }
    }
    registerTransportListeners() {
      const joinUnsub = this.transport.onPeerJoin((peerId) => {
        this.handlePeerJoin(peerId);
      });
      this.transportUnsubscribes.push(joinUnsub);
      const leaveUnsub = this.transport.onPeerLeave((peerId) => {
        this.handlePeerLeave(peerId, "disconnect");
      });
      this.transportUnsubscribes.push(leaveUnsub);
    }
    handlePeerJoin(playerId) {
      if (this.playerIds.has(playerId)) {
        return;
      }
      this.playerIds.add(playerId);
      if (!this.state) {
        this.initializeState([playerId]);
      }
      this.invokePlayerJoinHook(playerId);
      this.notifyPlayerJoin(playerId);
    }
    handlePeerLeave(playerId, reason) {
      if (!this.playerIds.has(playerId)) {
        return;
      }
      this.playerIds.delete(playerId);
      this.invokePlayerLeaveHook(playerId, reason);
      this.notifyPlayerLeave(playerId, reason);
    }
    invokePlayerJoinHook(playerId) {
      if (!this.gameLogic.onPlayerJoin || !this.state) {
        return;
      }
      const random2 = createPlayerRandom(this.tick, playerId);
      const time = this.tick * this.tickDuration;
      this.gameLogic.onPlayerJoin({
        game: this.state,
        playerId,
        random: random2,
        time
    ...
    invokePlayerLeaveHook(playerId, reason) {
      if (!this.state) {
        return;
      }
      if (this.gameLogic.onPlayerLeave) {
        ...
        this.removePlayerFromState(playerId);
      }
    }
    ...
    notifyPlayerJoin(playerId) {
      for (const callback of this.playerJoinCallbacks) {
        try {
          callback(playerId);
        } catch (error) {
          console.error("onPlayerJoin callback error:", error);
        }
      }
    }
    notifyPlayerLeave(playerId, reason) {
      for (const callback of this.playerLeaveCallbacks) {
        try {
          callback(playerId, reason);
        } catch (error) {
          console.error("onPlayerLeave callback error:", error);
        }
      }
    }
    /**
     * Start the simulation tick loop (host only)
     */
    start() {
    ...
    stop() {
      ...
      if (this.transportUnsubscribes.length) {
        for (const unsubscribe of this.transportUnsubscribes) {
          try {
            unsubscribe();
          } catch (error) {
            console.error("Transport unsubscribe error:", error);
          }
        }
    ...
    /**
     * Process a single tick (actions → systems → notify)
     */
    processTick() {
      if (!this.state) {
        return;
      ...
      this.tick++;
      this.actionExecutor.setTick(this.tick);
      this.systemExecutor.setTick(this.tick);
      const previousState = deepClone(this.state);
      if (this.gameLogic.systems) {
        this.systemExecutor.executeSystems(this.state, this.gameLogic.systems);
      }
      const stateChanged = !this.stateEquals(previousState, this.state);
      if (stateChanged) {
        this.revision++;
        this.notifyChange({
          predicted: false,
    ...
    /**
     * Dispatch an action (adds to queue and executes)
     */
    dispatchAction(actionName, playerId, payload) {
      const actionConfig = this.gameLogic.actions?.[actionName];
      if (!actionConfig) {
        console.warn(`Unknown action: ${actionName}`);
        return;
      }
      if (!this.state) {
        console.warn(`Cannot execute action "${actionName}" before state is initialized`);
        return;
      }
      const previousState = deepClone(this.state);
      const result = this.actionExecutor.executeAction(
        ...
        this.state,
        actionConfig
        // No actionIndex override - let executor auto-increment
      );
      if (result.rejected) {
        console.warn(`Action "${actionName}" rejected: ${result.reason}`);
        return;
      }
      const stateChanged = !this.stateEquals(previousState, this.state);
      if (stateChanged) {
    ...
    /**
     * Get current state (frozen read-only copy)
     */
    getState() {
      if (!this.state) {
        throw new Error("Game state not initialized yet");
      }
    ...
    subscribePlayerJoin(callback) {
      this.playerJoinCallbacks.push(callback);
      return () => {
        const index = this.playerJoinCallbacks.indexOf(callback);
        if (index >= 0) {
          this.playerJoinCallbacks.splice(index, 1);
        }
      };
    }
    subscribePlayerLeave(callback) {
      this.playerLeaveCallbacks.push(callback);
      return () => {
        const index = this.playerLeaveCallbacks.indexOf(callback);
        if (index >= 0) {
          this.playerLeaveCallbacks.splice(index, 1);
        }
      };
    }
    /**
    ...
    notifyChange(meta) {
      ...
      const frozenState = deepFreeze(deepClone(this.state));
      for (const callback of this.changeCallbacks) {
        try {
          callback(frozenState, meta);
        } catch (error) {
    ...
    /**
     * Simple state equality check (by value)
     */
    stateEquals(a, b) {
      if (a === b) {
        return true;
    ...
    getAPI() {
      const self2 = this;
      const actions = {};
      if (this.gameLogic.actions) {
        for (const actionName in this.gameLogic.actions) {
          actions[actionName] = (payload) => {
            const playerId = self2.transport.getPlayerId();
            self2.dispatchAction(actionName, playerId, payload);
          };
        }
      }
      const api = {
        actions,
        getState: () => self2.getState(),
        onChange: (callback) => self2.onChange(callback),
        onPlayerJoin: (callback) => self2.subscribePlayerJoin(callback),
        onPlayerLeave: (callback) => self2.subscribePlayerLeave(callback),
        getTick: () => self2.getTick(),
        getRevision: () => self2.getRevision(),
        random: new SeededRandom(self2.tick),
        myId: this.transport.getPlayerId(),
        isHost: this.isHost,
        playerIds: Array.from(this.playerIds),
        time: this.tick * this.tickDuration
      };
      Object.defineProperty(api, "playerIds", {
        enumerable: true,
        configurable: true,
        get: () => Array.from(self2.playerIds)
      });
      Object.defineProperty(api, "time", {
        enumerable: true,
        configurable: true,
        get: () => self2.tick * self2.tickDuration
      });
      Object.defineProperty(api, "myId", {
        enumerable: true,
        configurable: true,
        get: () => self2.transport.getPlayerId()
      });
      Object.defineProperty(api, "isHost", {
        enumerable: true,
        configurable: true,
        get: () => self2.isHost
      });
  ...
  // src/PhaserMultiplayerRuntime.ts
  var PhaserMultiplayerRuntime = class {
    constructor(gameLogic, transport, options) {
      this.transport = transport;
      this.trackedPlayers = /* @__PURE__ */ new Map();
      this.remotePlayers = /* @__PURE__ */ new Map();
      this.eventHandlers = /* @__PURE__ */ new Map();
      const extendedGameLogic = this.extendGameLogic(gameLogic);
      this.runtime = new MultiplayerRuntime(extendedGameLogic, transport, {
        isHost: transport.isHost(),
        playerIds: [transport.getPlayerId(), ...transport.getPeerIds()],
        tickRate: options?.tickRate ?? 30,
        devMode: options?.devMode ?? false
      });
      this.api = this.runtime.getAPI();
      this.api.onChange((state) => {
        this.updateRemotePlayers(state);
      });
      this.api.onPlayerJoin((playerId) => {
        this.handlePeerJoin(playerId);
      });
      this.api.onPlayerLeave((playerId) => {
        this.handlePeerLeave(playerId);
      });
    }
    /**
     * Track a local player sprite for automatic synchronization
     *
     * @param sprite - Phaser sprite or game object to sync
     * @param options - Tracking configuration
     */
    trackPlayer(sprite, options) {
      const playerId = this.transport.getPlayerId();
      if (!options.createRemotePlayer) {
        throw new Error("trackPlayer requires createRemotePlayer function");
      }
      this.trackedPlayers.set(playerId, { sprite, options });
      const updateRate = options.updateRate ?? 30;
      const intervalMs = 1e3 / updateRate;
      const updateIntervalId = setInterval(() => {
        this.syncLocalPlayer(playerId, sprite, options);
      }, intervalMs);
      this.trackedPlayers.get(playerId).updateIntervalId = updateIntervalId;
      this.syncLocalPlayer(playerId, sprite, options);
    }
    /**
     * Broadcast a game event to all players
     *
     * @param eventName - Event identifier
     * @param data - Event payload
     */
    broadcast(eventName, data) {
      this.api.actions.gameEvent({
        eventName,
        data,
        fromPlayerId: this.transport.getPlayerId(),
        timestamp: Date.now()
      });
    }
    /**
     ...
     */
    getPlayers() {
      return [this.transport.getPlayerId(), ...this.transport.getPeerIds()];
    }
    /**
     * Get the underlying MultiplayerRuntime API (for advanced use)
     */
    getAPI() {
      return this.api;
    }
    /**
     * Start the simulation tick loop (usually automatic)
     */
    start() {
      this.runtime.start();
    }
     ...
     */
    stop() {
      this.trackedPlayers.forEach((tracked) => {
        if (tracked.updateIntervalId) {
          clearInterval(tracked.updateIntervalId);
        }
      });
    ...
    // ============================================================================
    /**
     * Extend user's game logic with player tracking and events
     */
    extendGameLogic(gameLogic) {
      return {
        ...gameLogic,
        setup: (ctx) => {
          const userState = gameLogic.setup(ctx);
          return {
            ...userState,
            __martini: {
              players: Object.fromEntries(ctx.playerIds.map((id) => [id, { x: 0, y: 0, role: "" }])),
              events: [],
              eventClearCounter: 0
            }
          };
        ...
        actions: {
          ...gameLogic.actions,
          // Action for updating player position
          updatePlayerPosition: {
            ...
            apply: ({ game, playerId, input }) => {
              if (!game.__martini) game.__martini = { players: {}, events: [], eventClearCounter: 0 };
              if (!game.__martini.players[playerId]) {
                game.__martini.players[playerId] = {};
              }
              const player = game.__martini.players[playerId];
              player.x = input.x;
              player.y = input.y;
              player.velocityX = input.velocityX;
              player.velocityY = input.velocityY;
              player.frame = input.frame;
              player.role = input.role;
              game.__martini.eventClearCounter++;
              if (game.__martini.eventClearCounter >= 10) {
                game.__martini.events = [];
                game.__martini.eventClearCounter = 0;
              }
            }
          },
          // Action for broadcasting game events
          gameEvent: {
            input: {
              eventName: "string",
              data: "object",
            ...
            apply: ({ game, input }) => {
              if (!game.__martini) game.__martini = { players: {}, events: [], eventClearCounter: 0 };
              game.__martini.events.push(input);
            }
          }
        }
      };
     ...
     */
    syncLocalPlayer(playerId, sprite, options) {
      const syncProps = options.sync ?? ["x", "y", "velocityX", "velocityY", "frame"];
      const state = {
        x: sprite.x ?? 0,
        y: sprite.y ?? 0,
        velocityX: 0,
      ...
      if (sprite.body) {
        state.velocityX = sprite.body.velocity?.x ?? sprite.body.velocityX ?? 0;
        state.velocityY = sprite.body.velocity?.y ?? sprite.body.velocityY ?? 0;
      }
      if (sprite.frame && typeof sprite.frame.name !== "undefined") {
        state.frame = sprite.frame.name;
      }
      this.api.actions.updatePlayerPosition(state);
    }
    /**
     * Update remote player sprites from network state
     */
    updateRemotePlayers(state) {
      if (!state.__martini?.players) return;
      const myId = this.transport.getPlayerId();
      if (state.__martini.events && Array.isArray(state.__martini.events)) {
        for (const event of state.__martini.events) {
          const handlers = this.eventHandlers.get(event.eventName);
          if (handlers) {
            handlers.forEach((handler) => handler(event.fromPlayerId, event.data));
          }
        }
      }
      Object.entries(state.__martini.players).forEach(([playerId, playerData]) => {
        if (playerId === myId) return;
        let remoteSprite = this.remotePlayers.get(playerId);
        if (!remoteSprite && playerData.role) {
          const anyTracked = Array.from(this.trackedPlayers.values())[0];
          if (anyTracked && anyTracked.options.createRemotePlayer) {
            const scene = this.getSceneFromSprite(anyTracked.sprite);
            if (scene) {
              remoteSprite = anyTracked.options.createRemotePlayer(scene, playerData.role, {
                x: playerData.x,
                y: playerData.y,
                velocityX: playerData.velocityX,
                velocityY: playerData.velocityY
              });
              this.remotePlayers.set(playerId, remoteSprite);
            }
          }
        }
        if (remoteSprite) {
          remoteSprite.x = playerData.x;
          remoteSprite.y = playerData.y;
          if (remoteSprite.body) {
            remoteSprite.body.velocity.x = playerData.velocityX ?? 0;
            remoteSprite.body.velocity.y = playerData.velocityY ?? 0;
          }
          if (playerData.frame !== void 0 && remoteSprite.setFrame) {
            remoteSprite.setFrame(playerData.frame);
          }
        }
      });
    }
    /**
     * Handle peer join - create remote player sprite
     */
    handlePeerJoin(playerId) {
      console.log("[PhaserMultiplayerRuntime] Peer joined:", playerId);
    }
    /**
     * Handle peer leave - destroy remote player sprite
     */
    handlePeerLeave(playerId) {
      console.log("[PhaserMultiplayerRuntime] Peer left:", playerId);
      const remoteSprite = this.remotePlayers.get(playerId);
      if (remoteSprite && typeof remoteSprite.destroy === "function") {
        remoteSprite.destroy();
      }
      this.remotePlayers.delete(playerId);
    }
    /**
     * Extract scene from sprite (Phaser-specific)
     */
    getSceneFromSprite(sprite) {
      return sprite.scene || null;
    }
  };
...
`) && (e = e.replace(Kx, "%0A")), e.includes("\r") && (e = e.replace(Qx, "%0D")), e.includes("	") && (e = e.replace(Yx, "%09")), n.pathname = e, n;
  ...
  // ../../../node_modules/.pnpm/trystero@0.22.0_@multiformats+multiaddr@12.5.1/node_modules/trystero/src/utils.js
  ...
  // ../../../node_modules/.pnpm/trystero@0.22.0_@multiformats+multiaddr@12.5.1/node_modules/trystero/src/crypto.js
  var algo = "AES-GCM";
  var strToSha1 = {};
  ...
  // ../../../node_modules/.pnpm/trystero@0.22.0_@multiformats+multiaddr@12.5.1/node_modules/trystero/src/peer.js
  var iceTimeout = 5e3;
  ...
  var peer_default = (initiator, { rtcConfig, rtcPolyfill, turnConfig }) => {
    ...
    const waitForIceGathering = (pc3) => Promise.race([
      new Promise((res) => {
        const checkState = () => {
          if (pc3.iceGatheringState === "complete") {
            pc3.removeEventListener(iceStateEvent, checkState);
            res();
          }
        };
        pc3.addEventListener(iceStateEvent, checkState);
        checkState();
      }),
      new Promise((res) => setTimeout(res, iceTimeout))
    ]).then(() => ({
      type: pc3.localDescription.type,
      sdp: pc3.localDescription.sdp.replace(/a=ice-options:trickle\s\n/g, "")
    }));
    if (initiator) {
    ...
    pc2.onnegotiationneeded = async () => {
      try {
        ...
        const offer = await waitForIceGathering(pc2);
    ...
    pc2.onconnectionstatechange = () => {
      if (["disconnected", "failed", "closed"].includes(pc2.connectionState)) {
        handlers.close?.();
      }
    };
    pc2.ontrack = (e) => {
    ...
    return {
        ...
        try {
          if (sdp.type === offerType) {
            if (makingOffer || pc2.signalingState !== "stable" && !isSettingRemoteAnswerPending) {
              ...
              await all([
                pc2.setLocalDescription({ type: "rollback" }),
            ...
            await pc2.setLocalDescription();
            const answer = await waitForIceGathering(pc2);
      ...
      removeStream: (stream) => pc2.getSenders().filter((sender) => stream.getTracks().includes(sender.track)).forEach((sender) => pc2.removeTrack(sender)),
  ...
  // ../../../node_modules/.pnpm/trystero@0.22.0_@multiformats+multiaddr@12.5.1/node_modules/trystero/src/room.js
  ...
  // ../../../node_modules/.pnpm/trystero@0.22.0_@multiformats+multiaddr@12.5.1/node_modules/trystero/src/strategy.js
  var poolSize = 20;
  ...
  // ../../../node_modules/.pnpm/trystero@0.22.0_@multiformats+multiaddr@12.5.1/node_modules/trystero/src/mqtt.js
  var sockets = {};
  ...
  // ../transport-trystero/dist/TrysteroTransport.js
  var TrysteroTransport = class {
    constructor(options) {
      ...
      const cleanup = receive((message, peerId) => {
        if ("type" in message && message.type === "heartbeat") {
          if (this.currentHost !== message.sessionId) {
            this.currentHost = message.sessionId;
          }
        }
        if ("type" in message && message.type === "host_migration") {
          this.currentHost = message.newHost;
        }
  ...
  // browser.ts
  window.MartiniMultiplayer = {
    PhaserMultiplayerRuntime,
    TrysteroTransport
  };
